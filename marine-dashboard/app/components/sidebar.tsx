...
const Sidebar = () => {
  ...
  return (
    <motion.div
      className="fixed left-4 rounded-2xl shadow-lg overflow-hidden z-10"
    ...
    >
      <div className="p-4 h-full flex flex-col">
        <h1 className="text-2xl font-bold mb-8 text-white">{isHovered ? "Marines" : "M"}</h1>
        <nav className="flex-grow">
          <ul className="space-y-4">
            <SidebarItem icon={<Home />} text="Dashboard" href="/dashboard" />
            <SidebarItem icon={<Upload />} text="Upload" href="/dashboard/upload" />
            <SidebarItem icon={<Search />} text="Data Analysis" href="/search" />
...
const SidebarItem = ({ icon, text, href }: { icon: React.ReactNode; text: string; href: string }) => {
  ...
  return (
    <li onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <Link
        href={href}
        className="flex items-center space-x-4 text-white hover:bg-[#006A70] rounded-lg p-2 transition-colors duration-200"
      >
        {icon}
