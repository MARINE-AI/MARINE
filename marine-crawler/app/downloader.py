...
async def process_video_task(message_value: bytes):
    """Download and segment a video, then produce chunks to Kafka.
       If the downloaded file is too small, send it as a raw chunk.
    """
    try:
        ...
        # Use a persistent downloads directory in the project root.
        downloads_dir = os.path.join(os.getcwd(), "downloads")
        if not os.path.exists(downloads_dir):
            os.makedirs(downloads_dir)
            logger.info(f"Created downloads directory: {downloads_dir}")

        # Use yt-dlp's template so the native extension is preserved.
        ...
        # Wait a moment to ensure the file system has updated.
        await asyncio.sleep(2)
        # Use glob to search for the downloaded file matching the video_id.
        ...
        if file_size < MIN_VIDEO_SIZE:
            logger.warning("Downloaded video file is too small; sending the raw file as a single chunk.")
        ...
        for idx, chunk_file in enumerate(chunk_files):
            ...
            logger.info(f"Produced chunk {idx} for video {video_id}")
